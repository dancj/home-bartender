...
describe('breadcrumbTrail', () => {
  ...
  it('leaves the facet segment unlinked for the four facets with no index', () => {
    for (const key of ['spirit', 'occasion', 'tag', 'flavor'] as const) {
  ...
  it('always sets the root segment to All recipes → /', () => {
    for (const key of ['flavor', 'spirit', 'root', 'occasion', 'tag'] as const) {
