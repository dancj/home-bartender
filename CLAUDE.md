...
| Concern | File | Notes |
|---|---|---|
| Canonical taxonomy values | `data/taxonomy.yaml` | Single source of truth for every enum-shaped field (category, method, ice, format, glass, root, spirits, flavors, occasions). Edit here. |
...
- **Image fields** — `hero_image` and `gallery` are resolved through Astro's image pipeline (`astro:assets`, via the `image()` schema helper). Each recipe gets two shots: the cocktail (`<slug>.jpg` → `hero_image: ./slug.jpg`) and the ingredient lineup (`<slug>-ingredients.jpg` → `gallery: [./slug-ingredients.jpg]`), stored next to the recipe `.md`. Shoot the hero **3:4 portrait** (desktop hero/card frame is 4:5, slight trim). The lineup can be 3:4 portrait or landscape — landscape shows full column width on desktop and letterboxed in the 3:4 mobile swipe slide. To add photos, drop slug-named files (jpg/png/webp — not HEIC) in `intake/photos/` and run `npm run photos` (published recipes only; promote inbox drafts first): it resizes to 1600px, converts to JPEG, moves each file next to its recipe, and sets the frontmatter. It never commits; ship the batch via a normal PR. Leave empty (or omit) when there's no photo — the card/detail fall back to a masked glass-icon tile. Don't fabricate paths to files that don't exist (the build resolves them and will fail). `preparations` remains reserved and unused — leave it empty.
...
- The full Zod schema is enforced by `astro check` inside `npm run build`, which runs **only at the staging→main release build** (`deploy.yml`, on push to `main`). A draft still missing `blurb` / `glass` / `method` / `ice` breaks the release build. **So the reviewer must complete those (slugs per `data/taxonomy.yaml` / `TEMPLATE.md`) before the draft rides a release**, then promote via `npm run promote`.
...
- The enum dropdown options are **hardcoded in both `recipe.yml` and the workflow's `METHODS` / `ICES` / `GLASSES` arrays** — there's no codegen link to `data/taxonomy.yaml`. If you add a taxonomy value, update those lists too. `astro check` at release time is the real backstop, and the workflow filters any dropdown value not in its allow-list rather than writing a bad slug.
