...
import { categorizePr, sanitizeTitle, neutralizeClosingKeywords } from './releaseCategorize.mjs';
...
export function injectIntoBody(existingBody, managedBlock) {
  ...
  if (starts === 1 && ends === 1) {
    ...
    if (startIdx < endIdx) {
      ...
      // If the new block is identical to the old, return existingBody unchanged
      // (preserves byte-exact idempotency on no-op runs).
